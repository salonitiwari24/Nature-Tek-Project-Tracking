...
export function ProjectTable({
  ...
  const getClientTypeBadgeClass = (
    type:
      ProjectDetail['clientType']
  ) => {
    switch (type) {
      case 'INDIVIDUAL':
                ...
                return (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-4">
                      <span
                        className={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium uppercase ${getProjectTypeBadgeClass(
                          p.projectType
                        )}`}
                      >
                        {
                    ...
                    <td className="whitespace-nowrap px-4 py-4">
                      <span
                        className={`inline-flex rounded-md px-2 py-0.5 text-xs font-semibold uppercase ${getClientTypeBadgeClass(
                          p.clientType
                        )}`}
                      >
                        {
                    ...
                    <td className="px-4 py-4 text-xs font-medium text-zinc-500">
                     {
  LIFECYCLE_STAGE_LABELS[
    p.currentStage as keyof typeof LIFECYCLE_STAGE_LABELS
  ]
}
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-4">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider ${getStatusBadgeClass(
                          p.status
                        )}`}
                      >
                        {p.status}
